...
%DIF ADD causal_decision_making.tex   Fri Feb 23 10:00:55 2024
...
\DIFdelbegin \DIFdel{How to inform clinical decision-making using routine care data? First,
  ...
  on time-varying data, as in }\DIFdelend \DIFaddbegin

\DIFadd{Causal inference enables machine learning methods to estimate
  treatment effects of medical interventions from }\DIFaddend electronic health
records (EHRs)\DIFdelbegin \DIFdel{or claims. Here, we decompose these pitfalls in a three steps retrospective case-control study: choice of study design,
  confounding variables, and estimator. Studying }\DIFdelend \DIFaddbegin \DIFadd{. However, researchers should be wary of many pitfalls.
  Here we explain and study common pitfalls and potential solutions,
  estimating }\DIFaddend the effect of albumin on mortality in sepsis \DIFdelbegin \DIFdel{in the Medical Information Mart for }\DIFdelend \DIFaddbegin \DIFadd{using an
}\DIFaddend Intensive Care database (MIMIC-IV) \DIFdelbegin \DIFdel{, we }\DIFdelend \DIFaddbegin \DIFadd{and comparing to outcome data from randomized
  ...
  estimator. Results }\DIFaddend show that these steps are all important \DIFdelbegin \DIFdel{to build valid
  decision-making. In a tutorial spirit, we make all the code and data openly
  available}\DIFdelend \DIFaddbegin \DIFadd{for valid
  ...
  patients >60 years old, males, and patients with septic shock}\DIFaddend .

% Please keep the Author Summary between 150 and 200 words
% Use first person. PLOS ONE authors please skip this step. 
% Author Summary not valid for PLOS ONE submissions.   
\section*{Author summary}
\DIFdelbegin \DIFdel{How to inform clinical decision-making using routine care data? First, individualizing care calls for tailored prediction, eg with machine learning.
  Second, predictions must be associated with actionable interventions to avoid
  shortcuts or biases in the data . For a given treatment, a useful model would
  distinguish responders from non-responders. This requires modeling
  heterogeneity in causal inference, which faces many }\DIFdelend \DIFaddbegin

\DIFadd{Rich routine-care data, as EHR or claims, is useful to individualize decision
  ...
  intervention, leading to many analytical }\DIFaddend pitfalls, particularly \DIFdelbegin \DIFdel{so
  on }\DIFdelend \DIFaddbegin \DIFadd{in
}\DIFaddend time-varying data\DIFdelbegin \DIFdel{, as in electronic health records (EHRs) or claims. Here,
  we decompose these pitfalls in a three steps retrospective case-control study: choice of study design, confounding variables, and estimator. Studying the effect of albumin on
  mortality in sepsis in the
  Medical Information Mart for Intensive Care
  database (MIMIC-IV) }\DIFdelend \DIFaddbegin \DIFadd{. We study these in a tutorial spirit, making
  ...
  Studying choice of fluid resuscitation in sepsis}\DIFaddend , we show that \DIFdelbegin \DIFdel{these steps are all important to build valid
  decision-making. In a tutorial spirit, we make all the code and data openly
  available}\DIFdelend \DIFaddbegin \DIFadd{common
  mistakes in steps 1, 2, and 3 equally compromise causal validity}\DIFaddend .
...
\DIFdelbegin \DIFdel{How can data lead to useful clinical decisions? Prevention strategies using
  ...
  burden of cardiovascular diseases \mbox{%DIFAUXCMD
  ...
  . Addressing treatment heterogeneity is crucial to effective care}\DIFdelend \DIFaddbegin \DIFadd{Informing a care option extends beyond merely predicting the occurrence
  ...
  validity of the inferences; numerous considerations must be carefully addressed}\DIFaddend .

\paragraph{Individualized \DIFdelbegin \DIFdel{predictions are not enough}\DIFdelend \DIFaddbegin \DIFadd{Medicine and Machine Learning Challenges}\DIFaddend }
Machine learning \DIFdelbegin \DIFdel{is central to individualized medicine  . Machine learning models
  recently outperformed }\DIFdelend \DIFaddbegin \DIFadd{plays a pivotal role in individualized medicine  \mbox{%DIFAUXCMD
...
%
For instance, \DIFdelbegin \DIFdel{many }\DIFdelend \DIFaddbegin \DIFadd{numerous }\DIFaddend machine learning algorithms \DIFdelbegin \DIFdel{use }\DIFdelend \DIFaddbegin \DIFadd{rely on }\DIFaddend post-treatment information \cite{badgeley2019deep,obermeyer2019dissecting,yuan2021temporal,wong2021external}\DIFdelbegin \DIFdel{;
  a paradigmatic example being }\DIFdelend \DIFaddbegin \DIFadd{, exemplified by }\DIFaddend a diagnostic model for skin cancer \DIFdelbegin \DIFdel{relying }\DIFdelend \DIFaddbegin \DIFadd{that depends }\DIFaddend on surgical marks \cite{winkler2019association}. \DIFdelbegin \DIFdel{A motivating example on Intense }\DIFdelend \DIFaddbegin \DIFadd{For Intensive }\DIFaddend Care Unit data\DIFdelbegin \DIFdel{is provided in }%DIFDELCMD < \nameref{apd:fig:motivating_example}%%%
\DIFdelend \DIFaddbegin \DIFadd{, focus of our study, such information markedly improves mortality prediction (Figure S1 \ref{apd:fig:motivating_example}), but cannot inform decisions}\DIFaddend .

\paragraph{\DIFaddbegin \DIFadd{The Importance of }\DIFaddend Causal \DIFdelbegin \DIFdel{thinking is key to data-driven decision-making}\DIFdelend \DIFaddbegin \DIFadd{Reasoning in Data-Driven Decision-Making}\DIFaddend } \cite{prosperi2020causal}
While \DIFdelbegin \DIFdel{traditional }\DIFdelend \DIFaddbegin \DIFadd{conventional }\DIFaddend machine learning relies on \DIFdelbegin \DIFdel{data
  resulting from prior interventions }\DIFdelend \DIFaddbegin \DIFadd{retrospective to generate predictions of future effects }\DIFaddend \cite{plecko2022causal}, truly \DIFdelbegin \DIFdel{insightful }\DIFdelend \DIFaddbegin \DIFadd{informing }\DIFaddend decision-making \DIFdelbegin \DIFdel{support needs to contrast the }\DIFdelend \DIFaddbegin \DIFadd{needs a comparison of }\DIFaddend potential outcomes with and without the intervention\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. This involves }\DIFaddend estimating a causal effect\DIFdelbegin \DIFdel{as in Randomized Control Trials (RCTs )
  \mbox{%DIFAUXCMD
...
\paragraph{\DIFdelbegin \DIFdel{Grounding clinical decisions on data requires considerations scattered across different fields.}\DIFdelend \DIFaddbegin \DIFadd{How lack of individual treatment model limits a cardiovascular risk score}\DIFaddend }
...
\paragraph{\DIFadd{Multiple Perspectives on Evidence-based Decision Making}}
\DIFadd{Across different fields, existing literature has emphasized different challenges associated
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
  ...
  confounding variables and less so on those due to poor designs or }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
    \cite{von2007strengthening,benchimol2015reporting,hernan2020causal,schneeweiss2021conducting,zeng2022uncovering}}\hspace{0pt}%DIFAUXCMD
  ,
  there emphasis primarily lies on biases that arise from temporal effects \mbox{%DIFAUXCMD
    \cite{suissa2008immortal,Oke2021leadtimebias,fu2021timing,hernan2016specifying,wang2022understanding,Bankhead2017attritionbias} }\hspace{0pt}%DIFAUXCMD
  or confounding variables \mbox{%DIFAUXCMD
    \cite{greenland1999causal,vanderweele2019principles,loh2021confounder}}\hspace{0pt}%DIFAUXCMD
  , with
  relatively less attention to issues arising from }\DIFaddend estimator selection.
...
\DIFadd{In contrast, machine }\DIFaddend learning and causal inference literature \DIFdelbegin \DIFdel{focuses on
  estimators
  \mbox{%DIFAUXCMD
  ...
  . This literature may be opaque for some due to intricate mathematical details and unverifiable assumptions}\DIFaddend . Guidelines seldom
\DIFdelbegin \DIFdel{mention }\DIFdelend \DIFaddbegin \DIFadd{address }\DIFaddend time-related biases, \DIFdelbegin \DIFdel{frequent loopholes when data have }\DIFdelend \DIFaddbegin \DIFadd{or covariate aggregation which
  frequently emerge in datasets with }\DIFaddend temporal dependencies
\cite{suissa2008immortal,fu2021timing}. \DIFdelbegin \DIFdel{Here, putting all steps --choice of study design, confounders, estimator-- in a consistent framework (section
}\DIFdelend \DIFaddbegin \DIFadd{Recently, the machine learning community
...
\DIFadd{In this work, we bring together epidemiological concepts and
...
}\DIFaddend \nameref{sec:inference_flow})\DIFdelbegin \DIFdel{, we show they are equally important to ensure validity of the analysis. We compare the corresponding biasesby studying the effect }\DIFdelend \DIFaddbegin \DIFadd{-- highlights their equal importance in ensuring
...
\DIFaddend %
\DIFaddbegin \section*{\DIFadd{Step-by-step framework for robust decision-making from EHR data}}\label{sec:inference_flow}
\DIFaddend
...
\DIFdelbegin \section*{\DIFdel{Step-by-step framework for robust decision-making from EHR data}}%DIFAUXCMD
%DIFDELCMD < \label{sec:inference_flow}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Whether or not using machine learning, many pitfalls threaten an analysis'
...
\DIFdelendFL \DIFaddbeginFL \begin{table*}[b!]
  \DIFaddendFL \resizebox{\linewidth}{!}{%
...
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{cost-effectiveness, but not individual treatment effect}\DIFaddend .
