...
%DIF ADD causal_decision_making.tex         Tue Oct 29 18:00:08 2024
...
of medical interventions from electronic health records (EHRs). \DIFdelbegin \DIFdel{However, researchers should be wary of many pitfalls.
}\DIFdelend \DIFaddbegin \DIFadd{The prevalence
  of such observational data and the difficulty for randomized trial to cover all
  population/treatment relationships make these methods increasingly attractive
  for the study of causal effects.
}

\DIFaddend Here we explain and study common \DIFaddbegin \DIFadd{methodological }\DIFaddend pitfalls and potential
solutions\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{. We illustrate our framework for causal inference }\DIFaddend estimating the
effect of albumin on mortality in sepsis using an Intensive Care database
(MIMIC-IV) and comparing \DIFdelbegin \DIFdel{to outcome data }\DIFdelend \DIFaddbegin \DIFadd{credible analyses variations to results }\DIFaddend from randomized
controlled trials (RCT) as gold-standard.
\DIFaddbegin
...
from observational data (0\%, 95\% CI -1\% to 1\%, and 0\%, 95\% -5\% to 5\%,
respectively). \DIFdelbegin \DIFdel{The third stepis fitting outcome models, and }\DIFdelend \DIFaddbegin \DIFadd{As the third step, }\DIFaddend we assess the influence
...
}\nameref{apd:selection_flowchart} \DIFadd{makes the inclusion and exclusion for the PICOT
  choices explicit.
}\DIFaddend
...
\DIFaddbegin \DIFadd{Since the early 1980ies, researcher investigated the use of colloid fluids in
