...
%DIF ADD causal_decision_making.tex         Mon Oct 28 16:50:52 2024
...
variables and defining a suitable estimand or effect measure. \DIFaddbegin \DIFadd{Unconfoundedness --inclusion of all confounders in the analysis--
  is a strong hypothesis that can be hard to obtain in real applications. In these cases, sensitivity analyses for omited variable bias allow to test the robustness of the results to missing confounders \mbox{%DIFAUXCMD
    \cite{cinelli2020making}}\hskip0pt%DIFAUXCMD
  , proximal inference can be used to leverage proxy of unobserved confounders \mbox{%DIFAUXCMD
    \cite{tchetgen2024introduction}}\hskip0pt%DIFAUXCMD
  , and the presence of natural experiment might identify the desired causal effect without unconfoundedness \mbox{%DIFAUXCMD
    \cite[Chapter 5, 9]{wager2020stats}}\hskip0pt%DIFAUXCMD
  .
}
...
\subsection*{\DIFadd{Estimation}}\label{sec:estimation_mimic_iv}
...
\DIFaddbegin \DIFadd{Unconfoundedness is a strong assumption that might be violated in practice. The existance of residual bias trough unobserved confounders can be mitigated with different strategies. The }\textit{\DIFadd{omitted variable bias}} \DIFadd{framework encourages sensitivity analyses allowing to derive bounds on the causal estimate by making assumptions on the strengh of association of the omitted variable with both the treatment and the outcome. We refer to \mbox{%DIFAUXCMD
    \cite{cinelli2020making} }\hskip0pt%DIFAUXCMD
  for a clear introduction under linear assumption and to \mbox{%DIFAUXCMD
    \cite{chernozhukov2022long} }\hskip0pt%DIFAUXCMD
  for an extension to general non-linear settings. In case of strong unobserved confounders for which proxy variables can be measured, }\textit{\DIFadd{proximal inference}} \DIFadd{can be used to obtain identifiability \mbox{%DIFAUXCMD
    \cite{tchetgen2024introduction}}\hskip0pt%DIFAUXCMD
  . These methods require expert knowledge to classify the proxy between treatment and outcome proxy, then run two-stage regression to recover the causal effect. Lastly, natural exeperiments, when available, should be exploited to estimate causal effects without the need of unconfoundedness. Instrumental variable methods exploit randomness influancing the treatment but unrelated to the outcome to simulate a randomized experiment \mbox{%DIFAUXCMD
    \cite[chapter 9]{wager2020stats}}\hskip0pt%DIFAUXCMD
  . Regression discontinuity designs leverage discontinuous treatment assignment mechanisms with the assumption of a continuous outcome \mbox{%DIFAUXCMD
