...
%DIF DEL manuscript_second_submission.tex   Fri Oct 25 18:45:38 2024
%DIF ADD causal_decision_making.tex         Wed Oct 30 10:17:25 2024
...
of medical interventions from electronic health records (EHRs). \DIFaddbegin \DIFadd{The prevalence
...
for the study of causal effects. }\DIFaddend However, researchers should be wary of many
pitfalls.
\DIFdelbegin \DIFdel{Here we explain and study common pitfalls and potential solutions,
}\DIFdelend \DIFaddbegin 

\DIFadd{We propose and illustrate a framework for causal inference }\DIFaddend estimating the
effect of albumin on mortality in sepsis using an Intensive Care database
(MIMIC-IV) and comparing \DIFdelbegin \DIFdel{to outcome data from randomized
controlled trials (RCT) }\DIFdelend \DIFaddbegin \DIFadd{credible analyses variations to results from RCTs }\DIFaddend as gold-standard.
\DIFaddbegin 

\DIFaddend The first step is study design, using the target trial concept and the PICOT
...
We show that \DIFdelbegin \DIFdel{a }\DIFdelend too large treatment-initiation time induces immortal time bias.
...
from observational data\DIFdelbegin \DIFdel{(0\%, 95\% CI -1\% to 1\%, and 0\%, 95\% -5\% to 5\%,
respectively). The third stepis fitting outcome models, and }\DIFdelend \DIFaddbegin \DIFadd{. As the third step, }\DIFaddend we assess the influence
of multiple models with varying assumptions\DIFdelbegin \DIFdel{. A }\DIFdelend \DIFaddbegin \DIFadd{, showing that a }\DIFaddend doubly robust estimator (AIPW)
...
\DIFaddbegin \DIFadd{Without causal thinking, machine learning is not enough for optimal clinical
decision making for each patient. Our step-by-step analytic framework helps the
analyst avoid the many pitfalls of applying machine learning to EHR data and
build models that avoid shortcuts and extract the best decision-making evidence.
}

\DIFaddend % Please keep the Author Summary between 150 and 200 words
...
\DIFdel{The QRISK is a risk score widely used in the UK to assess 10-year and lifetime cardio-vascular risk \mbox{%DIFAUXCMD
\cite{hippisley2017development}}\hskip0pt%DIFAUXCMD
. It is well calibrated, recommended by the national regulatory agency \mbox{%DIFAUXCMD
\cite{guideline2023cardiovascular}}\hskip0pt%DIFAUXCMD
, and the National Institute for Health and Care Excellence (NICE) argues to use such a score to systematically screen cardio-vascular disease (CVD) risks and treat with statins individuals at risk \mbox{%DIFAUXCMD
\cite{guideline2023cardiovascular}}\hskip0pt%DIFAUXCMD
, especially if 10-year CVD risk exceeds 10\% \mbox{%DIFAUXCMD
\cite{rabar2014lipid}}\hskip0pt%DIFAUXCMD
.
A cost-effectiveness analysis guided the choice of the 10\% threshold to optimize quality-adjusted life years at the population level \mbox{%DIFAUXCMD
\cite{guthrie2023competing}}\hskip0pt%DIFAUXCMD
. The NICE analysis is based on evidence mostly from three trials, accounting for more than 70\% of the effect: LIPID \mbox{%DIFAUXCMD
\cite{long1998prevention} }\hskip0pt%DIFAUXCMD
(32\% of the effect), PROSPER \mbox{%DIFAUXCMD
\cite{shepherd2002pravastatin} }\hskip0pt%DIFAUXCMD
(22\% of the effect), ALLHAT-LLT \mbox{%DIFAUXCMD
\cite{antihypertensive2002major} }\hskip0pt%DIFAUXCMD
(23\% of the effect). All these trials focused on high-risk patients and secondary prevention. All studies operate under the assumption of a consistent statin effect (risk ratios constant) across different CVD risk groups, irrespective of the baseline CVD risk and LDL-cholesterol levels. This aligns with the outcomes of two meta-analyses which observed broadly similar effectiveness across risk levels \mbox{%DIFAUXCMD
\cite{brugts2009benefits}}\hskip0pt%DIFAUXCMD
, but is challenged by another study in primary prevention \mbox{%DIFAUXCMD
\cite{byrne2019statins}}\hskip0pt%DIFAUXCMD
. Meanwhile the US Preventive Services Task Force concluded that the evidence is insufficient to determine the balance of benefits and harms of statin use for the primary prevention of CVD events and mortality in adults 76 years and older \mbox{%DIFAUXCMD
\cite{chou2022statin}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  \paragraph{How lack of individual treatment model limits a cardiovascular risk
%DIF >  score} The QRISK is a risk score widely used in the UK to assess 10-year and
%DIF >  lifetime cardio-vascular risk \cite{hippisley2017development}. It is well
%DIF >  calibrated, recommended by the national regulatory agency
%DIF >  \cite{guideline2023cardiovascular}, and the National Institute for Health and
%DIF >  Care Excellence (NICE) argues to use such a score to systematically screen
%DIF >  cardio-vascular disease (CVD) risks and treat with statins individuals at risk
%DIF >  \cite{guideline2023cardiovascular}, especially if 10-year CVD risk exceeds
%DIF >  10\% \cite{rabar2014lipid}.  A cost-effectiveness analysis guided the choice
%DIF >  of the 10\% threshold to optimize quality-adjusted life years at the
%DIF >  population level \cite{guthrie2023competing}. The NICE analysis is based on
%DIF >  evidence mostly from three trials, accounting for more than 70\% of the
%DIF >  effect: LIPID \cite{long1998prevention} (32\% of the effect), PROSPER
%DIF >  \cite{shepherd2002pravastatin} (22\% of the effect), ALLHAT-LLT
%DIF >  \cite{antihypertensive2002major} (23\% of the effect). All these trials
%DIF >  focused on high-risk patients and secondary prevention. All studies operate
%DIF >  under the assumption of a consistent statin effect (risk ratios constant)
%DIF >  across different CVD risk groups, irrespective of the baseline CVD risk and
%DIF >  LDL-cholesterol levels. This aligns with the outcomes of two meta-analyses
%DIF >  which observed broadly similar effectiveness across risk levels
%DIF >  \cite{brugts2009benefits}, but is challenged by another study in primary
%DIF >  prevention \cite{byrne2019statins}. Meanwhile the US Preventive Services Task
%DIF >  Force concluded that the evidence is insufficient to determine the balance of
%DIF >  benefits and harms of statin use for the primary prevention of CVD events and
%DIF >  mortality in adults 76 years and older \cite{chou2022statin}.

\DIFdelbegin \paragraph{\DIFdel{Critics on the efficiency of systematic screening and treatment heterogeneity}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdel{Conversely, the World Health Organization (WHO) published a systematic review on screening for CVD risk in 2019 \mbox{%DIFAUXCMD
\cite{eriksen2021effectiveness} }\hskip0pt%DIFAUXCMD
asserting the limited effectiveness of such screenings, aligning with a Cochrane review that also reported poor effectiveness \mbox{%DIFAUXCMD
\cite{krogsboll2012general} }\hskip0pt%DIFAUXCMD
resulting in over-treatment. The surge in statin prescriptions has been linked to a divergence in statin prescriptions at specified risk score levels \mbox{%DIFAUXCMD
\cite{van2013efficiency}}\hskip0pt%DIFAUXCMD
...
}\DIFdelend %DIF >  \paragraph{Critics on the efficiency of systematic screening and treatment heterogeneity}
%DIF >  Conversely, the World Health Organization (WHO) published a systematic review on
%DIF >  screening for CVD risk in 2019 \cite{eriksen2021effectiveness} asserting the
%DIF >  limited effectiveness of such screenings, aligning with a Cochrane review that
%DIF >  also reported poor effectiveness \cite{krogsboll2012general} resulting in
%DIF >  over-treatment. The surge in statin prescriptions has been linked to a
%DIF >  divergence in statin prescriptions at specified risk score levels
%DIF >  \cite{van2013efficiency}. This leads to over-prescription for low-risk patients
%DIF >  and under-prescription for high-risk patients, casting doubt on the overall
%DIF >  efficacy as the constancy assumption of the statin treatment effect might be
%DIF >  violated. Accounting for sources of heterogeneity in patient profiles would be
%DIF >  instrumental in optimizing treatment allocation.

\DIFdelbegin \DIFdel{Moreover, the WHO report critiques suboptimal adoption leading to social biases in the screened populations. In the UK, there are instances of treatment heterogeneity at a given risk level \mbox{%DIFAUXCMD
\cite{van2013efficiency} }\hskip0pt%DIFAUXCMD
and documented evidence of social inequities in health checks \mbox{%DIFAUXCMD
\cite{krska2016implementation}}\hskip0pt%DIFAUXCMD
...
}\DIFdelend %DIF >  Moreover, the WHO report critiques suboptimal adoption leading to social biases
%DIF >  in the screened populations. In the UK, there are instances of treatment
%DIF >  heterogeneity at a given risk level \cite{van2013efficiency} and documented
%DIF >  evidence of social inequities in health checks \cite{krska2016implementation}.
%DIF >  Evidence suggests that predominantly socially advantaged patients undergo
%DIF >  screening, consequently being offered statin treatment. This critique further
%DIF >  emphasizes the presence of treatment heterogeneity.
...
\ref{table:picot}. \DIFaddbegin \DIFadd{A selection flowchart such as in
}\nameref{apd:selection_flowchart} \DIFadd{makes inclusion and exclusion choices for PICOT explicit.
}\DIFaddend 
...
\DIFadd{Unconfoundedness --inclusion of all confounders in the analysis-- is a strong
hypothesis that can be hard to obtain in real applications. In these cases,
sensitivity analyses for omitted variable bias allow to test the robustness of
the results to missing confounders \mbox{%DIFAUXCMD
...
, and the presence of natural experiment might
identify the desired causal effect without unconfoundedness \mbox{%DIFAUXCMD
\cite[Chapter 5,
  9]{wager2020stats}}\hskip0pt%DIFAUXCMD
.
}

\DIFaddend The
\emph{estimand} is the final \DIFdelbegin \DIFdel{statistical }\DIFdelend \DIFaddbegin \DIFadd{causal }\DIFaddend quantity estimated from the data.
...
\DIFaddbegin \paragraph{\DIFadd{Causal estimators}}

\DIFadd{A given estimand can be estimated through different methods. One can model the
outcome with regression models also known as
G-formula, \mbox{%DIFAUXCMD
\cite{robins1986role} }\hskip0pt%DIFAUXCMD
and use it as a predictive counterfactual model
...
Weighting (IPW) \mbox{%DIFAUXCMD
\cite{austin2015moving}}\hskip0pt%DIFAUXCMD
. Finally, doubly robust methods model
both the outcome and the treatment, benefiting from the convergence of both
models \mbox{%DIFAUXCMD
\cite{wager2020stats}}\hskip0pt%DIFAUXCMD
. There is a variety of doubly robust models,
reviewed in }\nameref{apd:causal_estimators}\DIFadd{.
}
...
%DIFDELCMD < %%%
\DIFdel{A given estimand can be estimated through different methods. One can model the
outcome with regression models also known as
G-formula, \mbox{%DIFAUXCMD
\cite{robins1986role} }\hskip0pt%DIFAUXCMD
and use it as a predictive counterfactual model
...
models \mbox{%DIFAUXCMD
\cite{wager2020stats}}\hskip0pt%DIFAUXCMD
. There is a variety of doubly robust models, reviewed in }%DIFDELCMD < \nameref{apd:causal_estimators}%%%
\DIFdelend \DIFaddbegin \DIFadd{Missing covariate values might also be a source of confounding. Some statistical
...
}\nameref{apd:statistical_considerations} \DIFadd{details general sanity check for
imputation strategies when using statistical estimators}\DIFaddend .

\paragraph{\DIFdelbegin \DIFdel{Estimation }\DIFdelend \DIFaddbegin \DIFadd{Statistical estimation }\DIFaddend models of outcome and treatment}
...
\DIFaddbegin \DIFadd{MIMIC-IV is a publicly available database that contains information from real
...
\subsection*{\DIFaddbegin \DIFadd{Step 1: }\DIFaddend Study design \DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{-- }\DIFaddend effect of crystalloids on mortality in sepsis}%
...
\begin{itemize}[leftmargin=2ex]
  \item \textcolor{P}{Population}: Patients with sepsis in an ICU stay according
        ...
        follow-up of at least 24 hours, and age over 18 years\DIFdelbegin \DIFdel{described in Appendix \ref{apd:table:albumin_for_sepsis:table1_complete}}\DIFdelend \DIFaddbegin \DIFadd{.
        }\nameref{apd:selection_flowchart} \DIFadd{details the selection flowchart and
        }\nameref{apd:albumin_for_sepsis:table1_complete} \DIFadd{the population
        characteristics}\DIFaddend .
...
patient characteristics\DIFdelbegin \DIFdel{and }%DIFDELCMD < \nameref{apd:selection_flowchart} %%%
\DIFdel{details the selection flowchart}\DIFdelend .


\begin{table}[h!]
  \centering\small
  \resizebox{\columnwidth}{!}{
    \input{img_main/albumin_for_sepsis__obs_1d__table1_24h.tex}%
  }
  \\[.5ex]
...
\DIFdelbegin \DIFdel{\ref{fig:causal_diagram_albumin}}\DIFdelend \DIFaddbegin \nameref{apd:causal_diagram_albumin}\DIFaddend . Gray confounders are not controlled for since
...
\DIFdelbegin \subsection*{\DIFdel{Estimation}}%DIFAUXCMD
%DIFDELCMD < \label{sec:estimation_mimic_iv}
%DIFDELCMD < 
...
implementations. \DIFaddbegin \nameref{apd:statistical_considerations} \DIFadd{details statistical considerations that we identify as important but missing in these packages, namely lack of cross fitting estimators, bad practices for imputation, lack of closed form confidence intervals.
}

\subsection*{\DIFadd{Step 3: Statistical estimation}}\label{sec:estimation_mimic_iv}


\paragraph{\DIFadd{Confounder aggregation:}}

\DIFadd{We tested multiple aggregations such as the last value before the start of the
...
features, categorical variables were one-hot encoded (thus discarded missing values).
}\DIFaddend 
...
\subsection*{\DIFaddbegin \DIFadd{Step 4: }\DIFaddend Vibration analysis \DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{-- }\DIFaddend Comparing sources of systematic errors}%
\label{sec:vibration_analysis_mimic_iv}


\paragraph{Study design \DIFaddbegin \DIFadd{flaw }\DIFaddend -- Illustration of immortal time bias:}
...
periods lead to albumin being markedly more efficient \DIFaddbegin \DIFadd{(detailed results with causal forest and other choices of aggregation in }\nameref{apd:detailed_results_itb}\DIFadd{)}\DIFaddend .
...
\paragraph{Confounder choice \DIFaddbegin \DIFadd{flaw}\DIFaddend } We consider other choice of confounding variables
...
variables, lead to sizeable bias. \DIFaddbegin \DIFadd{This is consistent with the literature
enhancing the importance of a clinically valid DAG \mbox{%DIFAUXCMD
\cite{greenland1999causal}}\hskip0pt%DIFAUXCMD
.
}\DIFaddend 

\paragraph{Estimation choices \DIFaddbegin \DIFadd{flaw }\DIFaddend -- Confounder aggregation, causal and nuisance estimators:}
...
detailed analysis showed little differences for other aggregations (\nameref{apd:detailed_results} \DIFaddbegin \DIFadd{for complete results, and }\nameref{apd:vibration_analysis_for_aggregation} \DIFadd{for a detailed study on aggregation choices}\DIFaddend ). Both methodological studies \cite{naimi2023challenges} and
consistency with published RCTs suggest to prefer doubly-robust approaches.

\subsection*{\DIFaddbegin \DIFadd{Step 5: }\DIFaddend Treatment heterogeneity \DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{-- }\DIFaddend Which treatment for a sub-population?}%
...
RCT  \cite{caironi2014albumin}. It is also beneficial for older patients (age >=60) and males. \DIFaddbegin \nameref{apd:hte} \DIFadd{details the heterogeneity analysis.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend % Importance of reference target trial
...
%DIF < 
%DIF <  No magic bullet and its not all about ML
%DIF >  Focus on the medical question
\DIFaddbegin 

\DIFadd{Since the early 1980ies, researcher investigated the use of colloid fluids in
...
\mbox{%DIFAUXCMD
\cite{xu2014comparison}}\hskip0pt%DIFAUXCMD
. As even large RCTs left unanswered questions,
researchers focused on meta-analyses. Here our analysis is in line with the
latest two meta-analyses \mbox{%DIFAUXCMD
\cite{xu2014comparison,li2020resuscitation}}\hskip0pt%DIFAUXCMD
, as we
...
\DIFaddbegin \DIFadd{The project was run on a laptop running Ubuntu 22.04.2 LTS with the following hardware: CPU 12th Gen Intel(R) Core(TM) i7-1270P with 16 threads and 15 GB of RAM.
}\DIFaddend 
...
%DIF <  \section*{Competing interests}
\DIFaddbegin \section*{\DIFadd{Acknowledgments}}

\DIFadd{We thank all the PhysioNet team for their encouragements and support. In
...
\begin{figure}[!h]
  \begin{subfigure}{\linewidth}
    \centering
    \includegraphics[width=0.8\linewidth]{img_supp/predictive_failure__pr_auc.pdf}
    \caption{Area under the Precision-Recall curve (PR\_AUC)}\label{apd:fig:motivating_example_pr_auc}
  \end{subfigure}
  \hfill
  \begin{subfigure}{\linewidth}
    \centering
    \includegraphics[width=0.8\linewidth]{img_supp/predictive_failure__roc_auc.pdf}
...
\begin{figure}[!h]
  ...
  \includegraphics[width=0.7\linewidth]{img_supp/catalog_of_bias_immortal_time_bias.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{img_supp/timeline_albumin_for_sepsis.pdf}
...
\begin{figure}[h!]
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=0.72\linewidth]{img_supp/confounder.pdf}
    \small\sffamily Confounder
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=0.72\linewidth]{img_supp/collider.pdf}
    \small\sffamily Collider
  \end{minipage}
  \hfill
  \begin{minipage}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=0.72\linewidth]{img_supp/instrumental_variable.pdf}
    \small\sffamily Instrumental \\variable
  \end{minipage}
  \vfill
  \begin{minipage}[t]{0.4\linewidth}
    \centering
    \includegraphics[width=.9\linewidth]{img_supp/mediator.pdf}%
    \\
  ...
  \begin{minipage}[t]{0.4\linewidth}
    \centering
    \includegraphics[width=\linewidth]{img_supp/effect_modifier.pdf}%
    % \llap{%
...
%DIFAUXCMD
%DIFDELCMD < \label{apd:ate_on_mimic}
%DIFDELCMD < {\bf %%%
\DIFdel{Estimation of Treatment effect with MIMIC data.}%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend %DIF >  \paragraph*{S1 Appendix.}
...
}\DIFdelend %DIF >  We searched for causal inference studies in MIMIC using PubMed and Google
...
\DIFdelbegin %DIFDELCMD < \begin{itemize}[leftmargin=2ex]
%DIFDELCMD <   \item %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{liu2021effects} }\hskip0pt%DIFAUXCMD
studied the effect of \textcolor{I}{High-flow
        ...
        main outcome \mbox{%DIFAUXCMD
\cite{stephan2015high,hernandez2016effect}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  \begin{itemize}[leftmargin=2ex]
...
\DIFdel{\mbox{%DIFAUXCMD
\cite{yarnell2023oxygenation} }\hskip0pt%DIFAUXCMD
...
}\DIFdelend %DIF >    \item \cite{yarnell2023oxygenation} studied the effect of \textcolor{I}{lower hypoxemia} vs \textcolor{C}{higher
...
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{hsu2015association} }\hskip0pt%DIFAUXCMD
studied the effect of \textcolor{I}{indwelling
        ...
        replicating the study is available }\href{https://github.com/alistairewj/mimic-iv-aline-study/blob/main/mimic_iv_aline.ipynb}{\DIFdel{here}}%DIFAUXCMD
\DIFdel{.
}\DIFdelend %DIF >    \item \cite{hsu2015association} studied the effect of \textcolor{I}{indwelling
...
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{feng2018transthoracic} }\hskip0pt%DIFAUXCMD
studied the effect of
        ...
        found a significant positive reduction of mortality (odd ratio 0.78, 95\% CI 0.68-0.90).
          ...
          open source}}%DIFAUXCMD
\DIFdel{.
}\DIFdelend %DIF >    \item \cite{feng2018transthoracic} studied the effect of
...
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{gani2023structural} }\hskip0pt%DIFAUXCMD
studied the effect of \textcolor{I}{liberal
        ...
        previous RCTs where no effects have been reported \mbox{%DIFAUXCMD
\cite{panwar2016conservative, mackle2019conservative}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >    \item \cite{gani2023structural} studied the effect of \textcolor{I}{liberal
...
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{shahn2020fluid} }\hskip0pt%DIFAUXCMD
studied the effect of \textcolor{I}{fluid-limiting
        ...
        \mbox{%DIFAUXCMD
\cite{corl2019restrictive} }\hskip0pt%DIFAUXCMD
and two previous meta-analyses
        \mbox{%DIFAUXCMD
\cite{malbrain2014fluid,meyhoff2020lower}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >    \item \cite{shahn2020fluid} studied the effect of \textcolor{I}{fluid-limiting
...
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{chinaeke2021impact} }\hskip0pt%DIFAUXCMD
studied the effect of \textcolor{I}{statin use
        ...
        \mbox{%DIFAUXCMD
\cite{lee2017preadmission,kyu2019preadmission}}\hskip0pt%DIFAUXCMD
. But all RCTs
        \mbox{%DIFAUXCMD
\cite{national2014rosuvastatin,singh2017effects} }\hskip0pt%DIFAUXCMD
found no improvement for sepsis (not
        pre-admission administration though). The \mbox{%DIFAUXCMD
\cite{wan2014effect}
        }\hskip0pt%DIFAUXCMD
...
\DIFdel{\mbox{%DIFAUXCMD
\cite{adibuzzaman2019323} }\hskip0pt%DIFAUXCMD
studied the effect of \textcolor{I}{higher} vs
        ...
        consistent results from a target trial \mbox{%DIFAUXCMD
\cite{national2004higher}}\hskip0pt%DIFAUXCMD
.
  }%DIFDELCMD < \item %%%
\DIFdel{\mbox{%DIFAUXCMD
\cite{adibuzzaman2019323} }\hskip0pt%DIFAUXCMD
also studied the effect of \textcolor{I}{early
        ...
        \mbox{%DIFAUXCMD
\cite{papazian2010neuromuscular}}\hskip0pt%DIFAUXCMD
.
  }%DIFDELCMD < \item %%%
\DIFdel{\mbox{%DIFAUXCMD
...
studied the administration of \textcolor{I}{a
        ...
        importance of initialization timing. This is consistent with \mbox{%DIFAUXCMD
\cite{xu2014comparison}}\hskip0pt%DIFAUXCMD
, who
        ...
        results are aligned with \mbox{%DIFAUXCMD
...
\DIFdel{\mbox{%DIFAUXCMD
\cite{wang2023early} }\hskip0pt%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelend %DIF >    \item \cite{chinaeke2021impact} studied the effect of \textcolor{I}{statin use
...
previous RCTs \mbox{%DIFAUXCMD
\cite{liu2021effects,shahn2020fluid,adibuzzaman2019323}}\hskip0pt%DIFAUXCMD
.
}%DIFDELCMD < \clearpage
%DIFDELCMD < %%%
\DIFdelend %DIF >  These eleven studies mainly used propensity score matching (6) and IPW (4), two of
...
\DIFaddbegin \DIFadd{Unconfoundedness is a strong assumption that might be violated in practice. The
existance of residual bias trough unobserved confounders can be mitigated with
different strategies. The }\textit{\DIFadd{omitted variable bias}} \DIFadd{framework encourages
sensitivity analyses allowing to derive bounds on the causal estimate by making
assumptions on the strengh of association of the omitted variable with both the
...
confounders for which proxy variables can be measured, }\textit{\DIFadd{proximal
  inference}} \DIFadd{can be used to obtain identifiability
\mbox{%DIFAUXCMD
\cite{tchetgen2024introduction}}\hskip0pt%DIFAUXCMD
. These methods require expert knowledge to
classify the proxy between treatment and outcome proxy, then run two-stage
regression to recover the causal effect. Lastly, natural exeperiments, when
available, should be exploited to estimate causal effects without the need of
unconfoundedness. Instrumental variable methods exploit randomness influancing
...
\begin{table}[]
  % \resizebox{\textwidth}{!}{%
  \centering\small
  \input{img_supp/compute_time_estimates_est_lr_rf_b_50.tex}
  % }
  \\
...
%DIFAUXCMD
%DIFDELCMD < \label{apd:computing_resources}
%DIFDELCMD < {\bf %%%
\DIFdel{Computing resources.}%DIFDELCMD < }
%DIFDELCMD < 
...
\begin{table}[h!]
  \resizebox{\textwidth}{!}{%
    \input{img_supp/albumin_for_sepsis__obs_1d__table1_24h_supplementary.tex}
  }\\
  \caption{Characteristics of the trial population measured on the first 24
    hours of ICU stay. \\
...
\begin{figure}[h!]
  \centering%
  \includegraphics[width=\linewidth]{img_supp/dagitty_graph.pdf}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{img_supp/main_vibration_analysis_supp.pdf}
...
deviation from graph \DIFdelbegin \DIFdel{\ref{fig:causal_diagram_albumin}}\DIFdelend \DIFaddbegin \nameref{apd:causal_diagram_albumin}\DIFaddend , the larger the bias
...
\begin{figure}[h!]
  ...
  \begin{subfigure}[b]{0.45\linewidth}
    \centering
    \includegraphics[width=\linewidth]{img_supp/sensitivity__ps_distribution__first_last_median__Forests.pdf}
    \caption{}\label{apd:fig:albumin_for_sepsis:overlap_measure_first_last_median}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.45\linewidth}
    \centering
    \includegraphics[width=\linewidth]{img_supp/sensitivity__ps_distribution__first__Forests.pdf}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\linewidth]{img_supp/boxplot_est__DML__nuisances__Forests__final_RF.pdf}
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{img_supp/cate_all_category_random_forest.pdf}
  \caption{Distribution of Conditional Average Treatment effects on sex, age,
...
%DIFAUXCMD
%DIFDELCMD < \label{apd:risk_assessment_scores}
%DIFDELCMD < {\bf %%%
\DIFdel{Risk assessment tools for prevention.}%DIFDELCMD < }
%DIFDELCMD < 
...
cardio-vascular risk \mbox{%DIFAUXCMD
\cite{hippisley2017development}}\hskip0pt%DIFAUXCMD
...
statins initiation \mbox{%DIFAUXCMD
\cite{guideline2023cardiovascular}}\hskip0pt%DIFAUXCMD
. The NICE argues in favor
of systematic screening of cardio-vascular risks to increase the number of
people receiving statins \mbox{%DIFAUXCMD
\cite{guideline2023cardiovascular}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  \paragraph*{S9 Appendix.}
...
NICE guidelines \mbox{%DIFAUXCMD
\cite{rabar2014lipid}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  \textbf{An example of nationally deployed cardiovascular prevention tool: QRISK}
...
on the cost of life-long statin treatments \mbox{%DIFAUXCMD
\cite{guthrie2023competing}}\hskip0pt%DIFAUXCMD
. This
study assumes constant statins effect among different CVD risk groups:
  ...
  different risk levels.}} %DIFAUXCMD
\DIFdel{This hypothesis is supported by the meta-analysis of
at least for the important risk factors of sex and age \mbox{%DIFAUXCMD
\cite{brugts2009benefits}}\hskip0pt%DIFAUXCMD
...
doubts about the overall efficacy of statins for primary prevention \mbox{%DIFAUXCMD
\cite{byrne2019statins}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  The QRISK is a risk score widely used in the UK to assess 10-year
...
history of CVD.
}\DIFdelend %DIF >  The QRISK seems to mostly inform the initiation of statins for patients
...
%DIFAUXCMD
\DIFdelend %DIF >  They base their evidence on a cost-effectiveness analysis choosing the 10\%
...
cardio-vascular risks from 2019 \mbox{%DIFAUXCMD
\cite{eriksen2021effectiveness} }\hskip0pt%DIFAUXCMD
is
...
\mbox{%DIFAUXCMD
\cite{diao2012pharmacotherapy}}\hskip0pt%DIFAUXCMD
...
heterogeneity at a given risk level \mbox{%DIFAUXCMD
\cite{van2013efficiency} }\hskip0pt%DIFAUXCMD
or documentation
of social inequities of health checks \mbox{%DIFAUXCMD
\cite{krska2016implementation}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  The updated guidelines from the US Preventive Services Task Force found
...
}\DIFdelend %DIF >  \textbf{Critics on the efficiency of systematic screening points to treatment heterogeneity}
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  On the other side, the WHO systematic review on systematic screening for
...
}\DIFdelend %DIF >    \item  The second critic points to a treatment allocation bias for risk
...
\item%DIFAUXCMD
\DIFdel{ALLHAT-LLT \mbox{%DIFAUXCMD
\cite{antihypertensive2002major} }\hskip0pt%DIFAUXCMD
...
%DIFDELCMD < \clearpage
%DIFDELCMD < %%%
\DIFdelend %DIF >  \textbf{NICE evidence for statin efficacy}

\DIFdelbegin \section*{\DIFdel{Acknowledgments}}
%DIFAUXCMD
\DIFdelend %DIF >  In the NICE analysis, three trials weight for more than 70\% of the effect. All
...
%DIFDELCMD < %%%
\DIFdelend % \begin{thebibliography}{10}
