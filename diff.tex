...
%DIF DEL original_manuscript.tex      Fri Feb  2 16:19:46 2024
%DIF ADD causal_decision_making.tex   Fri Feb  2 18:25:16 2024
...
%DIF >  Please keep the abstract below 300 words --> 235
\section*{Abstract}
\DIFdelbegin \DIFdel{How to inform clinical decision-making using routine care data? First,
individualizing care calls for tailored prediction, eg }\DIFdelend \DIFaddbegin 

\DIFadd{Causal inference frameworks allow to flexibly estimate treatment effects of
medical interventions }\DIFaddend with machine learning \DIFdelbegin \DIFdel{.
...
on time-varying data, as in electronic health records (EHRs)or claims. Here, we decompose these pitfalls in a three steps retrospective case-control study: choice of study design, confounding variables, and estimator. Studying the effect of albumin on
mortality in sepsis in the }\DIFdelend \DIFaddbegin \DIFadd{methods from electronic health
...
an example in }\DIFaddend Medical Information Mart for Intensive Care database (\DIFdelbegin \DIFdel{MIMIC-IV)}\DIFdelend \DIFaddbegin \DIFadd{MIMIC-IV) on
...
patients >60 years old, males, and patients with septic shock. In conclusion}\DIFaddend , we
show that these steps are all important to build \DIFaddbegin \DIFadd{a }\DIFaddend valid decision-making\DIFdelbegin \DIFdel{. In a tutorial spirit, we make all the code and data openly
available}\DIFdelend \DIFaddbegin \DIFadd{, causal
inference framework and that misspecifications can drastically skew the results}\DIFaddend .

% Please keep the Author Summary between 150 and 200 words
% Use first person. PLOS ONE authors please skip this step. 
% Author Summary not valid for PLOS ONE submissions.   
\section*{Author summary}
\DIFdelbegin \DIFdel{How }\DIFdelend \DIFaddbegin 

\DIFadd{Causal inference frameworks have the potential }\DIFaddend to inform clinical
decision-making using routine care data\DIFdelbegin \DIFdel{? First,
individualizing care calls for tailored prediction, eg with machine learning .
Second, predictions must be associated with actionable interventions to avoid
shortcuts or biases in the data . For a given treatment,
a useful model would
distinguish responders from non-responders. This requires modeling
heterogeneity in causal inference, which faces }\DIFdelend \DIFaddbegin \DIFadd{, especially where randomized controlled
...
assignment of interventions are confounded by indication, which introduces }\DIFaddend many
pitfalls, particularly \DIFdelbegin \DIFdel{so
on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend time-varying data\DIFdelbegin \DIFdel{, as in electronic health records (EHRs) or claims. Here, we decompose these pitfalls in a three stepsretrospective case-control study: choice of study design,
...
available}\DIFdelend \DIFaddbegin \DIFadd{. In a tutorial spirit, we make all
...
which includes exploring the treatment effect in subgroups}\DIFaddend .
...
\DIFdelbegin \DIFdel{How can data lead to useful clinical decisions? Prevention strategies using
well-validated and calibrated cardiovascular risk models have not reduced the burden of cardiovascular diseases \mbox{%DIFAUXCMD
\cite{eriksen2021effectiveness}}\hspace{0pt}%DIFAUXCMD
, probably because they targeted both
responders and non-responders  \mbox{%DIFAUXCMD
\cite{krska2016implementation}}\hspace{0pt}%DIFAUXCMD
. Addressing treatment heterogeneity is crucial to effective care}\DIFdelend \DIFaddbegin \DIFadd{Clinical equipoise extends beyond merely predicting the occurrence of an event;it involves estimating treatment effects in populations, particularly within subgroups. Randomized controlled trials (RCTs) are the gold standard to address these questions, but face issues of their own. In this context, causal inference frameworks emerge as an ideal tool to tackle such questions in observational data. However, numerous considerations must be carefully addressed to ensure the validity of the inferences obtained from these frameworks}\DIFaddend .

\paragraph{\DIFaddbegin \DIFadd{Beyond }\DIFaddend Individualized \DIFdelbegin \DIFdel{predictions are not enough}%DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD < %%%
\DIFdel{Machine learning is central to individualized medicine. Machine learning models
recently outperformed }\DIFdelend \DIFaddbegin \DIFadd{Predictions: Machine Learning Challenges in Medicine}} \DIFadd{\mbox{%DIFAUXCMD
\cite{rajkomar2018scalable,liu2019comparison,li2020behrt,beaulieu2021machine,aggarwal2021diagnostic}
}\hspace{0pt}%DIFAUXCMD
Machine learning plays a pivotal role in individualized medicine, showcasing superior performance over }\DIFaddend traditional rule-based clinical scores \DIFdelbegin \DIFdel{to predict }\DIFdelend \DIFaddbegin \DIFadd{in predicting }\DIFaddend a patient's readmission risk, mortality, or future comorbidities using Electronic Health Records (EHRs) \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{rajkomar2018scalable,
    liu2019comparison,li2020behrt,beaulieu2021machine, aggarwal2021diagnostic}}\hspace{0pt}%DIFAUXCMD
.
%DIF < 
But there is growing evidence }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{rajkomar2018scalable,liu2019comparison,li2020behrt,beaulieu2021machine,aggarwal2021diagnostic}}\hspace{0pt}%DIFAUXCMD
.
However, mounting evidence suggests }\DIFaddend that machine-learning models can \DIFdelbegin \DIFdel{reproduce and amplify biases }\DIFdelend \DIFaddbegin \DIFadd{inadvertently perpetuate and exacerbate biases present }\DIFaddend in the data \cite{rajkomar2018ensuring}, \DIFdelbegin \DIFdel{such as }\DIFdelend \DIFaddbegin \DIFadd{including }\DIFaddend gender or racial biases \cite{singh2022generalizability,gichoya2022ai}, \DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{and the }\DIFaddend marginalization of under-served populations \cite{seyyed2021underdiagnosis}. \DIFdelbegin \DIFdel{The models
typically encode these biases }\DIFdelend \DIFaddbegin \DIFadd{These biases are typically encoded }\DIFaddend by capturing shortcuts\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{—}\DIFaddend stereotypical or distorted features in the data \cite{geirhos2020shortcut,winkler2019association,degrave2021ai}.
%DIF < 
\DIFaddbegin 

\DIFaddend For instance, \DIFdelbegin \DIFdel{many }\DIFdelend \DIFaddbegin \DIFadd{numerous }\DIFaddend machine learning algorithms \DIFdelbegin \DIFdel{use }\DIFdelend \DIFaddbegin \DIFadd{rely on }\DIFaddend post-treatment information \cite{badgeley2019deep,obermeyer2019dissecting,yuan2021temporal,wong2021external}\DIFdelbegin \DIFdel{;
a paradigmatic example being }\DIFdelend \DIFaddbegin \DIFadd{, exemplified by }\DIFaddend a diagnostic model for skin cancer \DIFdelbegin \DIFdel{relying }\DIFdelend \DIFaddbegin \DIFadd{that depends }\DIFaddend on surgical marks \cite{winkler2019association}. A \DIFdelbegin \DIFdel{motivating example on Intense }\DIFdelend \DIFaddbegin \DIFadd{compelling illustration on Intensive }\DIFaddend Care Unit data is provided in \DIFdelbegin %DIFDELCMD < \nameref{apd:fig:motivating_example}%%%
\DIFdelend \DIFaddbegin \DIFadd{Figure S1 \ref{apd:fig:motivating_example}}\DIFaddend .

\paragraph{\DIFaddbegin \DIFadd{The Significance of }\DIFaddend Causal \DIFdelbegin \DIFdel{thinking is key to data-driven decision-making}\DIFdelend \DIFaddbegin \DIFadd{Reasoning in Data-Driven Decision-Making}\DIFaddend } \cite{prosperi2020causal}
While \DIFdelbegin \DIFdel{traditional }\DIFdelend \DIFaddbegin \DIFadd{conventional }\DIFaddend machine learning relies on \DIFdelbegin \DIFdel{data
resulting from prior interventions }\DIFdelend \DIFaddbegin \DIFadd{retrospective to generate predictions of future effects }\DIFaddend \cite{plecko2022causal}, truly insightful decision-making support \DIFdelbegin \DIFdel{needs to contrast the }\DIFdelend \DIFaddbegin \DIFadd{necessitates a comparison of }\DIFaddend potential outcomes with and without the intervention\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. This involves }\DIFaddend estimating a causal effect\DIFdelbegin \DIFdel{as in Randomized Control Trials (RCTs )
\mbox{%DIFAUXCMD
\cite{prosperi2020causal}}\hspace{0pt}%DIFAUXCMD
. Yet, RCTs face }\DIFdelend \DIFaddbegin \DIFadd{, mirroring the methodology employed in RCTs \mbox{%DIFAUXCMD
\cite{prosperi2020causal}}\hspace{0pt}%DIFAUXCMD
. However, RCTs encounter challenges such as }\DIFaddend selection biases \cite{travers2007external,averitt2020translating}, \DIFdelbegin \DIFdel{failure to
recruit minorities, or limited sample size }\DIFdelend \DIFaddbegin \DIFadd{difficulties in recruiting diverse populations, and limited sample sizes }\DIFaddend for exploring treatment heterogeneity across subgroups. Routinely collected data \DIFdelbegin \DIFdel{naturally provides }\DIFdelend \DIFaddbegin \DIFadd{presents }\DIFaddend a unique opportunity to assess real-life benefit-risk trade-offs associated with a decision \cite{desai2021broadening}, with \DIFdelbegin \DIFdel{less sampling bias , }\DIFdelend \DIFaddbegin \DIFadd{reduced sampling bias }\DIFaddend and sufficient data to capture heterogeneity \cite{rekkas2023standardized}. \DIFdelbegin \DIFdel{Estimating }\DIFdelend \DIFaddbegin \DIFadd{Nevertheless, estimating }\DIFaddend causal effects from \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{such }\DIFaddend data is challenging \DIFdelbegin \DIFdel{however, as the intervention is confounded }\DIFdelend \DIFaddbegin \DIFadd{due to the confounding of the intervention }\DIFaddend by indication. \DIFdelbegin \DIFdel{Hence}\DIFdelend \DIFaddbegin \DIFadd{Therefore}\DIFaddend , dedicated statistical techniques are \DIFdelbegin \DIFdel{needed }\DIFdelend \DIFaddbegin \DIFadd{imperative }\DIFaddend to emulate a "target trial" \cite{hernan2016specifying} from observational data.

%DIF <  Position of our paper
\paragraph{\DIFdelbegin \DIFdel{Grounding clinical decisions on data requires considerations scattered across different fields}\DIFdelend \DIFaddbegin \DIFadd{Population effect vs}\DIFaddend . \DIFaddbegin \DIFadd{individual treatment effect: QRISK cardiovascular risk tool }\DIFaddend }
\DIFaddbegin \DIFadd{The QRISK is a risk score widely used in the UK to assess 10-year and lifetime cardio-vascular risk \mbox{%DIFAUXCMD
\cite{hippisley2017development}}\hspace{0pt}%DIFAUXCMD
. It is well calibrated, recommended by the national regulatory agency \mbox{%DIFAUXCMD
\cite{guideline2023cardiovascular}}\hspace{0pt}%DIFAUXCMD
, and the National Institute for Health and Care Excellence (NICE) argues in favor of systematic screening of cardio-vascular disease (CVD) risks to increase the number of people receiving statins \mbox{%DIFAUXCMD
\cite{guideline2023cardiovascular}}\hspace{0pt}%DIFAUXCMD
, especially if 10-year CVD risk exceeds 10\% \mbox{%DIFAUXCMD
\cite{rabar2014lipid}}\hspace{0pt}%DIFAUXCMD
.
These suggestions found on a cost-effectiveness analysis selecting the 10\% threshold to optimize quality-adjusted life years at the population level \mbox{%DIFAUXCMD
\cite{guthrie2023competing}}\hspace{0pt}%DIFAUXCMD
. However, three trials in the NICE analysis weight for more than 70\% of the effect (LIPID \mbox{%DIFAUXCMD
\cite{long1998prevention} }\hspace{0pt}%DIFAUXCMD
(32\% of the effect), PROSPER \mbox{%DIFAUXCMD
\cite{shepherd2002pravastatin} }\hspace{0pt}%DIFAUXCMD
(22\% of the effect), ALLHAT-LLT \mbox{%DIFAUXCMD
\cite{antihypertensive2002major} }\hspace{0pt}%DIFAUXCMD
(23\% of the effect)). All focus on high-risk patients and secondary prevention. All studies operate under the assumption of a consistent statin effect (risk ratios constant) across different CVD risk groups, irrespective of the baseline CVD risk and LDL-cholesterol levels. This aligns with the outcomes of two meta-analyses which observed broadly similar effectiveness across risk levels \mbox{%DIFAUXCMD
\cite{brugts2009benefits}}\hspace{0pt}%DIFAUXCMD
, but is challenged by another study in primary prevention \mbox{%DIFAUXCMD
\cite{byrne2019statins}}\hspace{0pt}%DIFAUXCMD
...
\cite{chou2022statin}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 

\DIFdelbegin \DIFdel{Epidemiologic literature emphasizes the }\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Critics on the efficiency of systematic screening and treatment heterogeneity}}
\DIFadd{Conversely, the World Health Organization (WHO) published a systematic review on screening for CVD risk in 2019 \mbox{%DIFAUXCMD
\cite{eriksen2021effectiveness} }\hspace{0pt}%DIFAUXCMD
...
\cite{krogsboll2012general} }\hspace{0pt}%DIFAUXCMD
...
\cite{van2013efficiency}}\hspace{0pt}%DIFAUXCMD
...
\DIFadd{Moreover, the WHO report critiques suboptimal adoption leading to social biases in the screened populations. In the UK, there are instances of treatment heterogeneity at a given risk level \mbox{%DIFAUXCMD
\cite{van2013efficiency} }\hspace{0pt}%DIFAUXCMD
...
\paragraph{\DIFadd{Integrating Multidisciplinary Perspectives for Informed Clinical Decision-Making}}
\DIFadd{Existing literature has only partially delved into the challenges associated
with estimating treatment effects using observational data. While epidemiologic
studies underscore the importance of the }\DIFaddend target trial approach
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{von2007strengthening,benchimol2015reporting,hernan2020causal,schneeweiss2021conducting, zeng2022uncovering}}\hspace{0pt}%DIFAUXCMD
,
but focuses on biases due to
confounding variablesand less so on those due to poor designs or }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{von2007strengthening,benchimol2015reporting,hernan2020causal,schneeweiss2021conducting,zeng2022uncovering}}\hspace{0pt}%DIFAUXCMD
,
the emphasis primarily lies on biases stemming from confounding variables, with
comparatively less attention given to issues arising from }\DIFaddend estimator selection.
\DIFdelbegin \DIFdel{Machine }\DIFdelend \DIFaddbegin \DIFadd{Recent replications of RCTs using observational data did not explore the impact
of modern machine learning methods on the robustness of the results
\mbox{%DIFAUXCMD
\cite{schneeweiss2021conducting,wang2023emulation}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{In contrast, machine }\DIFaddend learning and causal inference literature \DIFdelbegin \DIFdel{focuses on estimators \mbox{%DIFAUXCMD
\cite{belloni2014high, chernozhukov2018double,shalit2016tutorial,
    sharma2018tutorial,moraffah2021causal}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{predominantly
concentrate on various estimators
\mbox{%DIFAUXCMD
\cite{belloni2014high,chernozhukov2018double,shalit2016tutorial,sharma2018tutorial,moraffah2021causal}
}\hspace{0pt}%DIFAUXCMD
: propensity score matching \mbox{%DIFAUXCMD
\cite{stuart2010matching}}\hspace{0pt}%DIFAUXCMD
, inverse probability
weighting \mbox{%DIFAUXCMD
\cite{austin2015moving}}\hspace{0pt}%DIFAUXCMD
, outcome models \mbox{%DIFAUXCMD
\cite{robins1986role}}\hspace{0pt}%DIFAUXCMD
, doubly
robust methods \mbox{%DIFAUXCMD
\cite{chernozhukov2018double} }\hspace{0pt}%DIFAUXCMD
or deep learning based models
\mbox{%DIFAUXCMD
\cite{johansson2022generalization}}\hspace{0pt}%DIFAUXCMD
. This literature is often inundated with
intricate mathematical details and unverifiable assumptions}\DIFaddend . Guidelines seldom
\DIFdelbegin \DIFdel{mention }\DIFdelend \DIFaddbegin \DIFadd{address }\DIFaddend time-related biases, \DIFdelbegin \DIFdel{frequent loopholes when data have }\DIFdelend \DIFaddbegin \DIFadd{or covariates aggregation at the stay level which
frequently emerge in datasets with }\DIFaddend temporal dependencies
\cite{suissa2008immortal,fu2021timing}. \DIFdelbegin \DIFdel{Here, putting all steps --choice }\DIFdelend \DIFaddbegin \DIFadd{Recent tendencies from this community
departed from EHR data to focus on simulated data, which may not capture the
complexities of real-world data \mbox{%DIFAUXCMD
\cite{schuler2017targeted,dorie2019automated,
  alaa2019validating, curth2021really}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{In this work, we amalgamate abstract epidemiological concepts with foundational
principles from statistical literature, adopting an empirical approach tailored
to the practical needs of applied researchers. By establishing a cohesive
framework that encompasses the choice }\DIFaddend of study design, \DIFdelbegin \DIFdel{confounders, estimator-- in a consistent framework (section
}\DIFdelend \DIFaddbegin \DIFadd{consideration of
confounders, and selection of estimators (refer to Section
}\DIFaddend \nameref{sec:inference_flow}), we \DIFdelbegin \DIFdel{show they are equally important to ensure }\DIFdelend \DIFaddbegin \DIFadd{highlight their equal importance in ensuring
the }\DIFaddend validity of the analysis. \DIFdelbegin \DIFdel{We compare the corresponding biasesby studying the effect }\DIFdelend \DIFaddbegin \DIFadd{To illustrate and compare biases, we investigate
the impact }\DIFaddend of albumin on sepsis mortality \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{using data from }\DIFaddend a publicly available
intensive care database\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend MIMIC-IV \DIFdelbegin \DIFdel{) }\DIFdelend \cite{johnson2020mimic} (section
\nameref{sec:application_on_mimic_iv}).
\DIFdelbegin \DIFdel{The main section's focus is on being accessible, while the appendices expand
technical
details .
}\DIFdelend \DIFaddbegin 

\DIFadd{The primary focus of the main section is on accessibility, with technical
...
choices in the different analytical steps \DIFaddbegin \DIFadd{recalled in Figure \ref{fig:study_summary}}\DIFaddend .

\DIFaddbegin \begin{figure}[h!]
  \centering
  \includegraphics[width=\linewidth]{img/applied_inference_flow.pdf}
  \caption{\textbf{\DIFaddFL{Application of the step-by-step framework on which resuscitation fluid to use.}}}\label{fig:study_summary}
\end{figure}


\DIFaddend \subsection*{Study design: effect of crystalloids on mortality in sepsis}%
...
treated with a combination of crystalloids and albumin\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{. Glycopeptide antibiotherapy was similar between both groups 51.8 for treated \% vs 51.5 \% for controls. Vasopressors (80.2\% vs 41.7\%) and ventilation (96.8\% vs 87.0\%) were more prevalent in the treated populations, underlying the overall higher severity of patients receiving albumin.
}\DIFaddend Table \ref{table:albumin_for_sepsis:table1_simple} details
patient characteristics and \nameref{apd:selection_flowchart} details the selection flowchart\DIFdelbegin \DIFdel{)}\DIFdelend .
...
moderately strong but modifiable risk factors as established \DIFdelbegin \DIFdel{on }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the Framingham
cohort \cite{brand1976multivariate}\DIFdelbegin \DIFdel{.
Cardio-vascular risk scores }\DIFdelend \DIFaddbegin \DIFadd{, while others }\DIFaddend such as the QRISK \DIFaddbegin \DIFadd{score }\DIFaddend are used to optimize population-wide \DIFdelbegin \DIFdel{cost
effectiveness, but do not inform which patient benefits most from a given
treatment (}%DIFDELCMD < \nameref{apd:risk_assessment_scores}%%%
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{cost-effectiveness, but individual treatment effect}\DIFaddend .
...
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{brugts2009benefits} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend at least for the important risk factors of sex and age \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{brugts2009benefits}}\hspace{0pt}%DIFAUXCMD
...
doubts about the overall efficacy of statins for primary prevention \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite[Table 1]{byrne2019statins}}\hspace{0pt}%DIFAUXCMD
...
\begin{itemize}
  \item The first critic shows that the augmentation of statin prescriptions
        ...
        and high risk patient under-prescribed, resulting in \DIFdelbegin \DIFdel{dubious }\DIFdelend \DIFaddbegin \DIFadd{questionable }\DIFaddend overall
        improvement. \DIFdelbegin \DIFdel{That suggests that some patients are responders and
          others are non-responders. }\DIFdelend In this case, the constant effect
        hypothesis of statin \DIFdelbegin \DIFdel{taken by }\DIFdelend \DIFaddbegin \DIFadd{proposed by the }\DIFaddend NICE cost-effectiveness study is \DIFdelbegin \DIFdel{false}\DIFdelend \DIFaddbegin \DIFadd{in doubt}\DIFaddend .
        ...
        heterogeneity directions\DIFdelbegin \DIFdel{but, we lack a RCT conducted on different
          risk levels to conclude}\DIFdelend \DIFaddbegin \DIFadd{, but we lack data from RCTs to draw such conclusions}\DIFaddend .
