...
%DIF ADD causal_decision_making.tex         Thu Oct 31 09:34:58 2024
...
of medical interventions from electronic health records (EHRs). \DIFaddbegin \DIFadd{The prevalence
  ...
  for studying causal effects. }\DIFaddend However, researchers should be wary of many
pitfalls.
...
(MIMIC-IV) and comparing \DIFdelbegin \DIFdel{to outcome data from randomized
  controlled trials (RCT) }\DIFdelend \DIFaddbegin \DIFadd{various sensitivity analyses to results from RCTs }\DIFaddend as gold-standard.
\DIFaddbegin
...
We show that \DIFdelbegin \DIFdel{a }\DIFdelend too large treatment-initiation \DIFdelbegin \DIFdel{time induces }\DIFdelend \DIFaddbegin \DIFadd{times induce }\DIFaddend immortal time bias.
...
\DIFadd{Unconfoundedness --inclusion of all confounders in the analysis-- is a strong
  ...
  , and the presence of a natural experiment or RCT might
...
}\nameref{apd:statistical_considerations} \DIFadd{details general sanity checks for
...
implementations. \DIFaddbegin \nameref{apd:statistical_considerations} \DIFadd{details statistical considerations that we identified as important but missing in these packages, namely lack of cross fitting estimators, bad practices for imputation, or lack of closed form confidence intervals.
}
...
variables, lead to sizeable bias. \DIFaddbegin \DIFadd{This is consistent with the literature
  highlighting the importance of a clinically valid DAG \mbox{%DIFAUXCMD
...
cohort \cite{brand1976multivariate}, \DIFdelbegin \DIFdel{while others such as the QRISK score are used to optimize }\DIFdelend \DIFaddbegin \DIFadd{or optimizing }\DIFaddend population-wide cost-effectiveness \DIFdelbegin \DIFdel{, but not }\DIFdelend \DIFaddbegin \DIFadd{instead of }\DIFaddend individual treatment effect.
